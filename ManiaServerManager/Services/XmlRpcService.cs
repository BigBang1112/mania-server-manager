using ManiaServerManager.XmlRpc;
...
internal interface IXmlRpcService : IDisposable
{
    event RemoteCallback? Callback;

    bool WaitingForMessages { get; }
    Task<object?[]> CallAsync(string methodName, object?[] methodParams, CancellationToken cancellationToken = default);
    Task<string> CallXmlAsync(string methodName, object?[] methodParams, CancellationToken cancellationToken = default);
    Task<object?[]> CallAsync(string methodName, CancellationToken cancellationToken = default);
    Task<string> CallXmlAsync(string methodName, CancellationToken cancellationToken = default);
    ValueTask<bool> ConnectAsync(string ip, int port = 5000, CancellationToken cancellationToken = default);
...
public sealed class XmlRpcService
{
    ...
    private ModifiedBinaryReader? reader;
    private ModifiedBinaryWriter? writer;

    public event RemoteCallback? Callback;

    public bool WaitingForMessages { get; private set; }

    private readonly ILogger<XmlRpcService> logger;

    public XmlRpcService(ILogger<XmlRpcService> logger)
    {
        this.logger = logger;
    }
    ...
    public async Task<string> CallXmlAsync(string methodName, object?[] methodParams, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogTrace("Generated XML for {MethodName} (in {ElapsedMilliseconds}ms): {XmlPayload}", methodName, watch.Elapsed.TotalMilliseconds, xmlPayload);
    ...
    private async Task<string> SendAndReceiveAsync(string methodName, string xmlPayload, CancellationToken cancellationToken)
    {
        ...
        logger.LogDebug("{MethodName} (0x{Handle}) response received (in {ElapsedMilliseconds}ms).", methodName, handle.ToString("x8"), watch.Elapsed.TotalMilliseconds);
    ...
    private static object?[] ReadXmlRpcParams(string xml, ref MiniXmlReader r)
    {
        if (!r.SkipStartElement("params"))
        {
            ...
            {
                throw new Exception("Fault is not dictionary, cannot gather details");
            }
            ...
            throw new Exception("Cannot gather fault details (faultString not found)");
        }
    ...
    private static Dictionary<string, object?> ReadXmlRpcStruct(ref MiniXmlReader r)
    {
        ...
        while (r.SkipStartElement("member"))
        {
            ...
            var memberName = r.ReadContent().ToString();
    ...
    private uint ReceiveXmlPayload(ModifiedBinaryReader r, out string payload)
    {
    ...
    private uint SendXmlPayload(string xmlPayload)
    {
        ...
        {
            throw new Exception("Invalid string buffering");
        }
    ...
    private ModifiedBinaryReader GetReader()
    {
        _ = tcp ?? throw new Exception("TCP connection not available");
        return reader ??= new ModifiedBinaryReader(tcp.GetStream());
    }

    private ModifiedBinaryWriter GetWriter()
    {
        _ = tcp ?? throw new Exception("TCP connection not available");
        return writer ??= new ModifiedBinaryWriter(tcp.GetStream());
    }
}
